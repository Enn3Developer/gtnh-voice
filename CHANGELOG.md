...
- Live audio device/HRTF control API (`AudioDeviceController`): hot-swap the capture (microphone) device without interrupting the voice session, and hot-swap the output device and/or HRTF mode (AUTO/ON/OFF) via an ordered playback-context rebuild that recreates each speaker's positioned AL source lazily without losing its decoder/jitter state; falls back to the default device (or AUTO HRTF) on failure instead of leaving audio dead, and persists the chosen input/output device and HRTF mode via new `voice.inputDevice`/`voice.outputDevice`/`voice.hrtfMode` config options. No settings GUI yet - exercised for now via three temporary debug keybinds (cycle input device, cycle output device, cycle HRTF mode) that a future GUI will replace
